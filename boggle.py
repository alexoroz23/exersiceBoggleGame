...
class Boggle():
    ...
    def __init__(self):
        ...
        # reads all the words from words.txt and store them in a list
        self.words = self.read_dict("words.txt")

    def read_dict(self, dict_path):
        """Read and return all words in dictionary."""

        # opens the dictionary at the specified path
        dict_file = open(dict_path)
        # Read each line of the file, remove any leading or trailing whitespace, and store each line as an element in a list
    ...
    def make_board(self):
        ...
        board = [] #creates an empty list to hold board

        for y in range(5): # iterate over the rows of the board
            row = [choice(string.ascii_uppercase) for i in range(5)] # create a row of random letters using uppercase ascii characters
            board.append(row)  # append the row to the board list

        return board  # return the completed board as a list of lists
    ...
    def check_valid_word(self, board, word):
        ...
        # return the result based on wether the word is in the dictionary and/or on the board  
